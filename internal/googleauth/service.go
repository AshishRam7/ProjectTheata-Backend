...
func NewService(dbService database.Service, cfg *config.Config) (Service, error) {
	...
	if cfg.TokenEncryptionKey == "" {
		return nil, errors.New("CRITICAL: TOKEN_ENCRYPTION_KEY is not set in the environment. This key is required to encrypt user tokens")
	}

	keyBytes, err := hex.DecodeString(cfg.TokenEncryptionKey)
	if err != nil {
		log.Printf("CRITICAL: Failed to hex-decode TOKEN_ENCRYPTION_KEY: %v. Please ensure it's a valid hex string (e.g., from 'openssl rand -hex 32').", err)
		return nil, fmt.Errorf("token encryption key is not a valid hex string: %w", err)
	}

	if len(keyBytes) != 32 {
		log.Printf("CRITICAL: Decoded TOKEN_ENCRYPTION_KEY must be 32 bytes long for AES-256, but got %d bytes. Key: '%s'", len(keyBytes), cfg.TokenEncryptionKey)
		return nil, fmt.Errorf("decoded token encryption key must be 32 bytes long, but got %d bytes", len(keyBytes))
	}

	log.Println("Successfully loaded and decoded TOKEN_ENCRYPTION_KEY (32 bytes).")
...
func (s *service) HandleCallback(w http.ResponseWriter, r *http.Request) {
	...
	if code == "" {
		...
		http.Redirect(w, r, fmt.Sprintf("%s/app?error=%s&error_description=%s", config.AppConfig.FrontendURL, errMsg, errDesc), http.StatusTemporaryRedirect)
		return
	}
	...
	if err != nil {
		...
		http.Redirect(w, r, fmt.Sprintf("%s/app?error=token_exchange_failed", config.AppConfig.FrontendURL), http.StatusTemporaryRedirect)
		return
	}
	...
	if err != nil {
		...
		http.Redirect(w, r, fmt.Sprintf("%s/app?error=token_encryption_failed", config.AppConfig.FrontendURL), http.StatusTemporaryRedirect)
		return
	}
	...
	if err != nil {
		...
		http.Redirect(w, r, fmt.Sprintf("%s/app?error=db_save_failed", config.AppConfig.FrontendURL), http.StatusTemporaryRedirect)
		return
	}
